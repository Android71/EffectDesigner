...
namespace ArtNet.Packets
{
    public class ArtNetDmxPacket : ArtNetPacket
    {
        ...
        {
            dmxData[index] = color.B;
            dmxData[index + 1] = color.R;
            dmxData[index + 2] = color.G;
        }

        public void SetColor(int ixStart, int count, Color color) //count <= 169
        {
            for (int i = 0; i <= count; i++)
            {
                SetColor(ixStart + i * 3, color);
            }
        }
    }
}
